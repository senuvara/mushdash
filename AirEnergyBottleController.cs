...
public class AirEnergyBottleController : BaseEnemyObjectController
{
	...
	private void RecoveryEffects()
	{
		...
		{
			Singleton<TaskStageTarget>.instance.AddScore(Mathf.RoundToInt(300f * Singleton<BattleProperty>.instance.heartNoteRate), m_MusicData.objId, m_MusicData.noteData.ibms_id, m_MusicData.isAir);
			Singleton<EventManager>.instance.Invoke("Battle/OnScoreGetAir", Singleton<TaskStageTarget>.instance.GetAddScore());
		}
		else
		{
			Singleton<EventManager>.instance.Invoke("Battle/OnHpGetAir", num);
	...
	public override void OnControllerAttacked(int result, bool isDeaded)
	{
		...
		{
			RecoveryEffects();
			SpineActionController.Play("note_out_p", idx);
			if (GameMusicScene.instance.animators.Length > idx)
			{
