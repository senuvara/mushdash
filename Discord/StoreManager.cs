...
namespace Discord
{
	public class StoreManager
	{
		...
		private static void FetchSkusCallbackImpl(IntPtr ptr, Result result)
		{
			...
			FetchSkusHandler fetchSkusHandler = (FetchSkusHandler)gCHandle.Target;
			gCHandle.Free();
		...
		{
			GCHandle gCHandle = GCHandle.FromIntPtr(ptr);
			FetchEntitlementsHandler fetchEntitlementsHandler = (FetchEntitlementsHandler)gCHandle.Target;
			gCHandle.Free();
		...
		{
			GCHandle gCHandle = GCHandle.FromIntPtr(ptr);
			StartPurchaseHandler startPurchaseHandler = (StartPurchaseHandler)gCHandle.Target;
			gCHandle.Free();
