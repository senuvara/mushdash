...
namespace Discord
{
	public class StorageManager
	{
		...
		private static void ReadAsyncCallbackImpl(IntPtr ptr, Result result, IntPtr dataPtr, int dataLen)
		{
			...
			ReadAsyncHandler readAsyncHandler = (ReadAsyncHandler)gCHandle.Target;
			gCHandle.Free();
		...
		{
			GCHandle gCHandle = GCHandle.FromIntPtr(ptr);
			ReadAsyncPartialHandler readAsyncPartialHandler = (ReadAsyncPartialHandler)gCHandle.Target;
			gCHandle.Free();
			byte[] array = new byte[dataLen];
			Marshal.Copy(dataPtr, array, 0, dataLen);
		...
		{
			GCHandle gCHandle = GCHandle.FromIntPtr(ptr);
			WriteAsyncHandler writeAsyncHandler = (WriteAsyncHandler)gCHandle.Target;
			gCHandle.Free();
