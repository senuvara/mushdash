...
namespace Rewired.Data
{
	public class UserDataStore_PlayerPrefs : UserDataStore
	{
		...
		private int LoadControllerMaps(int playerId, ControllerType controllerType, int controllerId)
		{
			...
			List<SavedControllerMapData> allControllerMapsXml = GetAllControllerMapsXml(player, true, controller);
				...
				foreach (Player allPlayer2 in ReInput.players.AllPlayers)
				{
					...
					for (int i = 0; i < playerInfo.joystickCount; i++)
					{
						...
						if (joystick != null)
						{
							...
							allPlayer2.controllers.AddController(joystick, false);
						}
					}
				}
					...
					foreach (Player allPlayer3 in ReInput.players.AllPlayers)
					{
						...
						for (int j = 0; j < playerInfo2.joystickCount; j++)
						{
							...
							else
							{
								...
								if (!TryFindJoysticksImprecise(joystickInfo, out matches))
								{
									continue;
								}
							...
							allPlayer3.controllers.AddController(joystick2, false);
						}
					}
				}
			}
		...
		{
			string controllerMapPlayerPrefsKey = GetControllerMapPlayerPrefsKey(player, saveData.controller, saveData.categoryId, saveData.layoutId, true);
			PlayerPrefs.SetString(controllerMapPlayerPrefsKey, saveData.map.ToXmlString());
			controllerMapPlayerPrefsKey = GetControllerMapKnownActionIdsPlayerPrefsKey(player, saveData.controller, saveData.categoryId, saveData.layoutId, true);
		...
		private string GetJoystickCalibrationMapPlayerPrefsKey(Joystick joystick)
		{
			...
			str = str + "|controllerType=" + joystick.type;
		...
		{
			string controllerMapPlayerPrefsKey = GetControllerMapPlayerPrefsKey(player, controller, categoryId, layoutId, true);
			if (!PlayerPrefs.HasKey(controllerMapPlayerPrefsKey))
			{
				...
				controllerMapPlayerPrefsKey = GetControllerMapPlayerPrefsKey(player, controller, categoryId, layoutId, false);
				if (!PlayerPrefs.HasKey(controllerMapPlayerPrefsKey))
				{
					return string.Empty;
				}
		...
		private List<int> GetControllerMapKnownActionIds(Player player, Controller controller, int categoryId, int layoutId)
		{
			...
			string controllerMapKnownActionIdsPlayerPrefsKey = GetControllerMapKnownActionIdsPlayerPrefsKey(player, controller, categoryId, layoutId, true);
			if (!PlayerPrefs.HasKey(controllerMapKnownActionIdsPlayerPrefsKey))
			{
				...
				controllerMapKnownActionIdsPlayerPrefsKey = GetControllerMapKnownActionIdsPlayerPrefsKey(player, controller, categoryId, layoutId, false);
				if (!PlayerPrefs.HasKey(controllerMapKnownActionIdsPlayerPrefsKey))
				{
					return list;
				}
