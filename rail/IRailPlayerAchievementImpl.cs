...
namespace rail
{
	public class IRailPlayerAchievementImpl : RailObject, IRailPlayerAchievement, IRailComponent
	{
		...
		public virtual RailResult GetAchievementInfo(string name, out string achievement_info)
		{
			...
			try
			{
				return (RailResult)RAIL_API_PINVOKE.IRailPlayerAchievement_GetAchievementInfo(swigCPtr_, name, intPtr);
			}
			finally
			{
		...
		{
			IntPtr intPtr = (names != null) ? RAIL_API_PINVOKE.new_RailArrayRailString__SWIG_0() : IntPtr.Zero;
			try
			{
