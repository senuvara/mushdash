...
public class CreditScroller : MonoBehaviour
{
	...
	private void Update()
	{
		if (letsGo && !touchInside)
		{
			Vector2 velocity = srComponent.velocity;
			if ((double)Mathf.Abs(velocity.y) <= 0.1)
			{
				letsGo = false;
				PressReleased();
			}
		...
		if (Mathf.Abs(Input.GetAxis("Vertical")) > 0f || Mathf.Abs(Singleton<InputManager>.instance.RewiredGetAxisRaw("Vertical")) > 0f)
		{
			CancelInvoke("PressReleased");
			CancelInvoke("InitialEaseIn");
			scrollbarTween.Kill();
			isAutoScrolling = false;
			touchInside = true;
			if ((double)Input.GetAxis("Vertical") > 0.5 || (double)Singleton<InputManager>.instance.RewiredGetAxisRaw("Vertical") > 0.5)
			{
				scrollbar.value += 0.003f;
			}
			if ((double)Input.GetAxis("Vertical") < -0.5 || (double)Singleton<InputManager>.instance.RewiredGetAxisRaw("Vertical") < -0.5)
			{
				scrollbar.value -= 0.003f;
		...
		else if (touchInside && !Input.GetMouseButton(0))
		{
			tweenFloat = scrollbar.value;
			letsGo = true;
			touchInside = false;
