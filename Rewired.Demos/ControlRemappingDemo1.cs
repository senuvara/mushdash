	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		private void DrawPlayerSelector()
		{
			...
			foreach (Player player in ReInput.players.GetPlayers(true))
			{
				...
				bool flag2 = GUILayout.Toggle(flag, (!(player.descriptiveName != string.Empty)) ? player.name : player.descriptiveName, "Button", GUILayout.ExpandWidth(false));
				if (flag2 != flag && flag2)
				{
					selectedPlayer = player;
		...
		private void DrawMouseAssignment()
		{
			...
			bool flag2 = GUILayout.Toggle(flag, "Assign Mouse", "Button", GUILayout.ExpandWidth(false));
			if (flag2 != flag)
			{
				if (flag2)
				{
		...
		private void DrawJoystickSelector()
		{
			...
			bool flag2 = GUILayout.Toggle(flag, "None", "Button", GUILayout.ExpandWidth(false));
			if (flag2 != flag)
			{
			...
			if (selectedPlayer != null)
			{
				foreach (Joystick joystick in ReInput.controllers.Joysticks)
				{
					...
					flag2 = GUILayout.Toggle(flag, joystick.name, "Button", GUILayout.ExpandWidth(false));
					if (flag2 != flag)
					{
		...
		private void DrawControllerSelector()
		{
			...
			bool flag2 = GUILayout.Toggle(flag, "Keyboard", "Button", GUILayout.ExpandWidth(false));
			if (flag2 != flag)
			{
				selectedController.Set(0, ControllerType.Keyboard);
				ControllerSelectionChanged();
			...
			flag2 = GUILayout.Toggle(flag, "Mouse", "Button", GUILayout.ExpandWidth(false));
			if (flag2 != flag)
			{
		...
		private void DrawCalibrateButton()
		{
			...
			else if (GUILayout.Button("Calibrate " + controller.name, GUILayout.ExpandWidth(false)))
			{
		...
		private void DrawMapCategories()
		{
			...
			foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
			{
				...
				bool flag2 = GUILayout.Toggle(flag, (!(userAssignableMapCategory.descriptiveName != string.Empty)) ? userAssignableMapCategory.name : userAssignableMapCategory.descriptiveName, "Button", GUILayout.ExpandWidth(false));
				if (flag2 != flag)
				{
					selectedMapCategoryId = userAssignableMapCategory.id;
					selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, userAssignableMapCategory.id);
		...
		private void DrawInvertButton(int playerId, InputAction action, Pole actionAxisContribution, ControllerSelection controller, ControllerMap controllerMap, ActionElementMap elementMap)
		{
			...
			bool flag = GUILayout.Toggle(invert, "Invert", GUILayout.ExpandWidth(false));
			if (flag != invert)
			{
		...
		private void DrawCalibrationWindow(string title, string message)
		{
			...
			for (int i = 0; i < axisElementIdentifiers.Count; i++)
			{
				...
				bool flag2 = GUILayout.Toggle(flag, controllerElementIdentifier.name, "Button", GUILayout.ExpandWidth(false));
				if (flag != flag2)
				{
			...
			if (calibration.selectedElementIdentifierId >= 0)
			{
				...
				GUILayout.Space(15f);
				bool flag3 = GUILayout.Toggle(axis.enabled, "Enabled", "Button", GUILayout.ExpandWidth(false));
				if (axis.enabled != flag3)
				{
					axis.enabled = flag3;
				}
				GUILayout.Space(10f);
				bool flag4 = GUILayout.Toggle(calibration.recording, "Record Min/Max", "Button", GUILayout.ExpandWidth(false));
				...
				bool flag5 = GUILayout.Toggle(axis.invert, "Invert", "Button", GUILayout.ExpandWidth(false));
